      ...
      <div style={{maxWidth:'1100px',margin:'0 auto',padding:'2rem'}}>
        ...
        <div style={{marginBottom:'2rem',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px'}}>
          <div>
            ...
            <p style={{fontSize:'14px',color:'#6b7280'}}>Last updated: {new Date().toLocaleString()}</p>
          </div>
        </div>
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))',gap:'12px',marginBottom:'1.5rem'}}>
              <div style={{...cardStyle,textAlign:'center'}}>
                ...
                <p style={{fontSize:'12px',color:'#6b7280',margin:'0'}}>{stats.proUsers} of {stats.totalUsers} upgraded</p>
                <div style={{marginTop:'10px',background:'rgba(0,0,0,0.2)',borderRadius:'20px',height:'6px',overflow:'hidden'}}>
              ...
              <div style={{...cardStyle,textAlign:'center'}}>
                ...
                <p style={{fontSize:'12px',color:'#6b7280',margin:'0'}}>{stats.activeUsers} generated 1+ listing</p>
                <div style={{marginTop:'10px',background:'rgba(0,0,0,0.2)',borderRadius:'20px',height:'6px',overflow:'hidden'}}>
                  <div style={{height:'100%',background:'#3b82f6',width:`${stats.activationRate}%`,borderRadius:'20px'}}/>
                </div>
              </div>
              <div style={{...cardStyle,textAlign:'center'}}>
                <p style={{fontSize:'11px',fontWeight:'700',color:'#6b7280',letterSpacing:'1px',margin:'0 0 8px'}}>TRIAL → PRO TARGET</p>
                <p style={{fontSize:'2.5rem',fontWeight:'700',color:'#d4af37',margin:'0 0 4px'}}>5%</p>
                <p style={{fontSize:'12px',color:'#6b7280',margin:'0'}}>Industry avg is 2-5%</p>
                <div style={{marginTop:'10px',background:'rgba(0,0,0,0.2)',borderRadius:'20px',height:'6px',overflow:'hidden'}}>
            ...
            <div style={{marginBottom:'12px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'10px'}}>
              <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                {[
                  ...
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{padding:'6px 14px',borderRadius:'8px',border:'1px solid',fontSize:'12px',cursor:'pointer',fontWeight: activeTab === tab.key ? '600' : '400',
              ...
              <input
                ...
                style={{padding:'8px 14px',background:'rgba(0,0,0,0.3)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'8px',fontSize:'13px',color:'#f0f0f0',outline:'none',width:'220px'}}
              />
            </div>
            ...
            <div style={cardStyle}>
              <div style={{overflowX:'auto'}}>
                      ...
                      return (
                        <tr key={user.id} style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}}
                          ...
                          <td style={{padding:'10px 12px'}}>
                            <p style={{fontSize:'13px',color:'#f0f0f0',margin:'0',fontWeight:'500'}}>{user.full_name || '—'}</p>
                          ...
                          <td style={{padding:'10px 12px'}}>
                            <span style={{fontSize:'11px',fontWeight:'600',padding:'2px 10px',borderRadius:'20px',
                              background: user.plan === 'pro' ? 'rgba(29,158,117,0.2)' : inTrial ? 'rgba(212,175,55,0.15)' : 'rgba(255,255,255,0.05)',
                              color: user.plan === 'pro' ? '#1D9E75' : inTrial ? '#d4af37' : '#6b7280',
                              border: user.plan === 'pro' ? '1px solid rgba(29,158,117,0.3)' : inTrial ? '1px solid rgba(212,175,55,0.3)' : '1px solid rgba(255,255,255,0.08)'}}>
                              {user.plan === 'pro' ? '⭐ Pro' : inTrial ? '⏰ Trial' : 'Free'}
                            </span>
                          </td>
                          <td style={{padding:'10px 12px',fontSize:'13px',color: (user.listings_used || 0) > 0 ? '#f59e0b' : '#444',fontWeight: (user.listings_used||0) > 0 ? '600' : '400'}}>{user.listings_used || 0}</td>
                          <td style={{padding:'10px 12px',fontSize:'13px',color: (user.rewrites_used || 0) > 0 ? '#8b5cf6' : '#444'}}>{user.rewrites_used || 0}</td>
                          <td style={{padding:'10px 12px',fontSize:'13px',color: user.marketing_emails ? '#1D9E75' : '#444'}}>{user.marketing_emails ? '✅' : '—'}</td>
                          <td style={{padding:'10px 12px',fontSize:'12px',color: user.portfolio_slug ? '#1D9E75' : '#444'}}>{user.portfolio_slug || '—'}</td>
                          <td style={{padding:'10px 12px',fontSize:'12px',color:'#6b7280',whiteSpace:'nowrap'}}>
                ...
                {filteredUsers.length === 0 && (
                  <p style={{textAlign:'center',color:'#6b7280',padding:'2rem',fontSize:'13px'}}>No users found</p>
                )}
              </div>
            </div>
          </div>
