...
export default function ListingRescuePage() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(form.address || form.city || 'Listing Rescue', margin, 30)
    doc.setFontSize(9)
    ...
    outputSections.forEach(({ key, label }) => {
    ...
    addFooter()
    const safe = (form.address || form.city || 'Untitled').replace(/[^a-zA-Z0-9]/g, '-')
    doc.save(`ListingRescue-${safe}.pdf`)
  }
