...
export default function DemoLockedCard({ reason, toolName, usedTool }: Props) {
  ...
  const message = reason === 'paid_feature'
    ? `${toolName} uses premium AI services. Sign up free to unlock — no credit card required for the first 24 hours of Pro.`
    : `You already tried ${usedTool ? usedTool.replace(/-/g, ' ') : 'a tool'} in this demo session. Sign up free to unlock all 30+ tools — no credit card required for the first 24 hours of Pro.`
  ...
  return (
    <div style={{ background: 'var(--lw-card)', border: '2px solid #1D9E75', borderRadius: '12px', padding: '40px 32px', textAlign: 'center', maxWidth: '520px', margin: '40px auto' }}>
      ...
      <div style={{ fontSize: '12px', color: 'var(--lw-text-muted)', marginTop: '12px' }}>24 hours of full Pro access · No credit card</div>
    </div>
  )
}
