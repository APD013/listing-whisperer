...
export default function PricingPage() {
  ...
  return (
    <main style={{minHeight:'100vh',background:'var(--lw-bg)',fontFamily:"var(--font-plus-jakarta), sans-serif"}}>
      ...
      {/* NAV */}
      <nav style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'1rem 2rem',borderBottom:'1px solid var(--lw-border)',background:'rgba(255,255,255,0.9)',backdropFilter:'blur(10px)',position:'sticky',top:0,zIndex:100}}>
        <a href="/" style={{fontSize:'18px',fontWeight:'700',textDecoration:'none',color:'var(--lw-text)'}}>
          Listing<span style={{color:'#1D9E75'}}>Whisperer</span>
        </a>
        <div style={{display:'flex',gap:'16px',alignItems:'center'}}>
          <a href="/dashboard" style={{fontSize:'13px',color:'var(--lw-text-muted)',textDecoration:'none'}}>Dashboard</a>
          <a href="/login" style={{fontSize:'13px',color:'var(--lw-text-muted)',textDecoration:'none'}}>Sign In</a>
        </div>
      </nav>
      ...
      <div style={{maxWidth:'760px',margin:'0 auto',padding:'4rem 2rem',textAlign:'center'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))',gap:'1.5rem',marginBottom:'3rem',textAlign:'left'}}>
          ...
          {/* FREE TRIAL */}
          <div style={{background:'var(--lw-card)',borderRadius:'20px',border:'1px solid var(--lw-border)',padding:'2rem',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
            ...
            <a href="/signup"
              style={{display:'block',textAlign:'center',padding:'12px',borderRadius:'10px',border:'1px solid var(--lw-border)',color:'var(--lw-text-muted)',textDecoration:'none',fontSize:'14px',fontWeight:'500',background:'var(--lw-input)'}}>
              Start free trial
            </a>
          </div>

          {/* PRO */}
          <div style={{background:'var(--lw-card)',borderRadius:'20px',border:'2px solid #1D9E75',padding:'2rem',boxShadow:'0 4px 24px rgba(29,158,117,0.12)',position:'relative'}}>
            ...
            <h3 style={{fontSize:'18px',fontWeight:'700',marginBottom:'4px',color:'var(--lw-text)'}}>Pro</h3>
            <p style={{color:'#1D9E75',fontSize:'13px',marginBottom:'1.5rem'}}>For active agents</p>
            <p style={{fontSize:'2.5rem',fontWeight:'800',marginBottom:'4px',color:'var(--lw-text)',letterSpacing:'-1px'}}>$20<span style={{fontSize:'16px',fontWeight:'400',color:'var(--lw-text-muted)'}}>/mo</span></p>
        ...
        <div style={{textAlign:'left',maxWidth:'600px',margin:'0 auto'}}>
          ...
          {[
            ...
            <div key={q} style={{borderBottom:'1px solid var(--lw-border)',paddingBottom:'1.25rem',marginBottom:'1.25rem'}}>
              <p style={{fontWeight:'600',fontSize:'14px',marginBottom:'6px',color:'var(--lw-text)'}}>{q}</p>
              <p style={{fontSize:'13px',color:'var(--lw-text-muted)',lineHeight:'1.8',margin:'0'}}>{a}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{marginTop:'2rem',padding:'1.5rem',background:'rgba(29,158,117,0.08)',borderRadius:'12px',border:'1px solid rgba(29,158,117,0.15)'}}>
          <p style={{fontSize:'14px',color:'var(--lw-text)',margin:'0 0 4px',fontWeight:'600'}}>🎁 Early Agent Discount</p>
          <p style={{fontSize:'13px',color:'var(--lw-text-muted)',margin:'0'}}>Use code <strong style={{color:'#d4af37'}}>WELCOME50</strong> at checkout for 50% off your first Pro month.</p>
        </div>

        {/* VIRTUAL STAGING CREDITS */}
        <div style={{marginTop:'3rem',textAlign:'center'}}>
          <h2 style={{fontSize:'1.25rem',fontWeight:'700',color:'var(--lw-text)',marginBottom:'8px'}}>Need more Virtual Stagings?</h2>
          <p style={{fontSize:'14px',color:'var(--lw-text-muted)',marginBottom:'2rem'}}>Pro includes 3 per month. Buy more anytime — credits never expire.</p>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))',gap:'1rem',textAlign:'center'}}>
            {[
              ...
              <a key={pack.credits} href="/virtual-staging" style={{textDecoration:'none'}}>
                <div style={{background:'var(--lw-card)',borderRadius:'16px',border:'1px solid rgba(29,158,117,0.25)',padding:'1.5rem',transition:'all 0.2s',cursor:'pointer'}}
                  ...
                  <p style={{fontSize:'20px',fontWeight:'800',color:'var(--lw-text)',margin:'0 0 4px'}}>{pack.price}</p>
                  <p style={{fontSize:'14px',fontWeight:'700',color:'#1D9E75',margin:'0 0 6px'}}>{pack.credits}</p>
                  <p style={{fontSize:'12px',color:'var(--lw-text-muted)',margin:'0'}}>{pack.perUnit}</p>
                </div>
              </a>
            ))}
          </div>
        ...
        <p style={{fontSize:'12px',color:'var(--lw-text-muted)',marginTop:'2rem'}}>
          <a href="/terms" style={{color:'var(--lw-text-muted)'}}>Terms of Service</a> · <a href="/privacy" style={{color:'var(--lw-text-muted)'}}>Privacy Policy</a> · <a href="/faq" style={{color:'var(--lw-text-muted)'}}>FAQ</a>
        </p>
      </div>
    </main>
  )
