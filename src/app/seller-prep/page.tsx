...
export default function SellerPrepPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(form.address || form.neighborhood || 'Untitled Property', margin, 30)
    doc.setFontSize(9)
    doc.setTextColor(130, 130, 170)
    const dateStr = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    const agentStr = form.agentName ? `  ·  Agent: ${form.agentName}` : ''
    ...
    const sections = [
      ...
      { key: 'cma_analysis', label: 'CMA & Pricing' },
    ...
    addFooter()
    const safe = (form.address || form.neighborhood || 'Untitled').replace(/[^a-zA-Z0-9]/g, '-')
    doc.save(`SellerPrep-${safe}.pdf`)
  }
