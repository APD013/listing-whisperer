...
export default function LaunchKitPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(form.neighborhood || form.city || '7-Day Launch Kit', margin, 30)
    doc.setFontSize(9)
    ...
    const sections = [
      { key: 'day1', label: 'Day 1 — Launch Day' },
      { key: 'day2', label: 'Day 2 — Follow Up' },
      { key: 'day3', label: 'Day 3 — Mid Week' },
      { key: 'day4', label: 'Day 4 — Spotlight' },
      { key: 'day5', label: 'Day 5 — Open House' },
      { key: 'day6', label: 'Day 6 — Weekend' },
      { key: 'day7', label: 'Day 7 — Final Push' },
      { key: 'email_sequence', label: 'Email Sequence' },
      { key: 'social_calendar', label: 'Social Calendar' },
      { key: 'pro_tips', label: 'Pro Tips' },
    ]

    sections.forEach(({ key, label }) => {
      const content = launchPlan[key] || ''
      if (!content) return
    ...
    addFooter()
    const safe = (form.neighborhood || form.city || 'Untitled').replace(/[^a-zA-Z0-9]/g, '-')
    doc.save(`LaunchKit-${safe}.pdf`)
  }
