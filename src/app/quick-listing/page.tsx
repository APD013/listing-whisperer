...
export default function QuickListingPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.text('ListingWhisperer', margin, 18)
    doc.setFontSize(11)
    doc.setTextColor(180, 180, 210)
    doc.text(answers.neighborhood || answers.city || 'Quick Listing', margin, 30)
    doc.setFontSize(9)
    doc.setTextColor(130, 130, 170)
    const dateStr = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    const agentStr = brandVoice?.agentName ? `  ·  Agent: ${brandVoice.agentName}` : ''
    doc.text(`${dateStr}${agentStr}`, margin, 39)
    y = 56

    const addFooter = () => {
      doc.setFontSize(8)
      doc.setTextColor(150, 150, 150)
      doc.text('Generated by ListingWhisperer.com', pageW / 2, pageH - 8, { align: 'center' })
    }

    const sections = [
      { key: 'mls_standard', label: 'MLS Standard' },
    ...
    sections.forEach(({ key, label }) => {
      ...
      if (!content) return
      if (y > pageH - 40) { addFooter(); doc.addPage(); y = 20 }
      ...
      const lines: string[] = doc.splitTextToSize(content, contentW)
      lines.forEach((line: string) => {
        if (y > pageH - 18) { addFooter(); doc.addPage(); y = 20 }
    ...
    addFooter()
    const safe = (answers.neighborhood || answers.city || 'Untitled').replace(/[^a-zA-Z0-9]/g, '-')
    doc.save(`QuickListing-${safe}.pdf`)
  }
