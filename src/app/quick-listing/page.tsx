  ...
  const handleGenerate = async () => {
    ...
    try {
      const property = {
        ...
        beds: answers.beds || aiAnalysis?.beds || '',
        ...
        notes: `AI detected features from photos: ${aiAnalysis?.features || 'none'}. Additional context: ${aiAnalysis?.additional_notes || 'none'}`,
          ...
          <div>
            <div style={{textAlign:'center',marginBottom:'2rem'}}>
              ...
              <p style={{fontSize:'14px',color:'#6b7280',lineHeight:'1.7'}}>
                Upload 1-5 property photos. Our AI reads the photos and writes your full listing automatically. You just answer 3 quick questions.
              </p>
            </div>
        ...
        {step === 'questions' && (
          <div>
            ...
            {photos.length > 0 && (
              <div style={{display:'flex',gap:'8px',marginBottom:'1.5rem',flexWrap:'wrap'}}>
                {photos.slice(0,5).map((photo, i) => (
                  <img key={i} src={photo} alt="" style={{width:'70px',height:'70px',objectFit:'cover',borderRadius:'10px',border:'2px solid #1D9E75'}}/>
                ))}
              </div>
            )}
            ...
            <div style={{...cardStyle, marginBottom:'1rem'}}>
              <label style={{fontSize:'13px',fontWeight:'600',color:'#1D9E75',display:'block',marginBottom:'10px'}}>
                1️⃣ What's the listing price?
              </label>
              <input
                placeholder="e.g. $899,000"
                value={answers.price}
            ...
            <div style={{...cardStyle, marginBottom:'1rem'}}>
              <label style={{fontSize:'13px',fontWeight:'600',color:'#1D9E75',display:'block',marginBottom:'10px'}}>
                2️⃣ What's the neighborhood or city?
              </label>
              <input
                placeholder="e.g. Newport Beach, CA"
                value={answers.neighborhood}
                onChange={e => setAnswers({...answers, neighborhood: e.target.value})}
                style={inputStyle}
              />
            </div>
            ...
            <div style={{...cardStyle, marginBottom:'1.5rem'}}>
              <label style={{fontSize:'13px',fontWeight:'600',color:'#1D9E75',display:'block',marginBottom:'10px'}}>
                3️⃣ How many beds and baths?
              </label>
              <input
                placeholder="e.g. 3 bed / 2 bath"
                value={answers.beds}
                onChange={e => setAnswers({...answers, beds: e.target.value})}
                style={inputStyle}
              />
            </div>
            ...
            <p style={{fontSize:'11px',color:'#444',textAlign:'center',marginTop:'10px'}}>
              Generates 11 formats — MLS, Social, Email, Flyer & more
            </p>
          </div>
        )}
        ...
        {step === 'generating' && (
          <div style={{textAlign:'center',padding:'3rem 0'}}>
            ...
            <div style={{...cardStyle, padding:'1rem'}}>
              {['🏠 Analyzing property features...','✍️ Writing MLS description...','📸 Crafting Instagram captions...','📧 Composing email blast...','📄 Building flyer copy...'].map((step, i) => (
