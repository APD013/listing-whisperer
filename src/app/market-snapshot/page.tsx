...
export default function MarketSnapshotPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(form.neighborhood || form.state || 'Market Snapshot', margin, 30)
    doc.setFontSize(9)
    doc.setTextColor(130, 130, 170)
    const dateStr = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    doc.text(dateStr, margin, 39)
    y = 56
    ...
    const sections = [
      ...
      { key: 'price_trends', label: 'Price Trends' },
      { key: 'inventory_analysis', label: 'Inventory Analysis' },
      { key: 'buyer_seller_assessment', label: 'Buyer / Seller Assessment' },
      { key: 'client_email', label: 'Client Email' },
    ]

    sections.forEach(({ key, label }) => {
      const content = (result as any)[key] || ''
      if (!content) return
    ...
    addFooter()
    const safe = (form.neighborhood || form.state || 'Untitled').replace(/[^a-zA-Z0-9]/g, '-')
    doc.save(`MarketSnapshot-${safe}.pdf`)
  }
