...
export default function MarketSnapshotPage() {
  ...
  const loadHistory = async (uid: string) => {
    const { data } = await supabase
      ...
      .select('id, neighborhood, created_at, outputs')
      .eq('user_id', uid)
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '2rem 1.5rem', position: 'relative', zIndex: 1 }}>
        ...
        {result && !loading && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <ToolHandoff from="market-snapshot" handoffs={[
        ...
        <div style={{ marginTop: '2rem' }}>
          <p style={sectionHeadStyle}>Past Snapshots</p>
          {!historyLoaded ? null : history.length === 0 ? (
            <div style={{ ...s.card, textAlign: 'center', color: 'var(--lw-text-muted)', fontSize: '13px' }}>
              Your past snapshots will appear here.
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {history.map(item => (
                <div key={item.id} style={{ background: 'var(--lw-card)', border: '1px solid var(--lw-border)', borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ margin: 0, fontSize: '13px', fontWeight: '600', color: 'var(--lw-text)' }}>{item.neighborhood}</p>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      onClick={() => { setResult(item.outputs as SnapshotResult); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await supabase.from('market_snapshots').delete().eq('id', item.id)
                        if (userId) loadHistory(userId)
                      }}
